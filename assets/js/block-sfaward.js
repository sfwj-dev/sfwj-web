...
registerBlockType( 'sfwj/nominees', {
	...
	edit( { attributes, setAttributes } ) {
		const SpreadSheetUrl = () => {
			return (
				<TextControl
					label={ __( 'スプレッドシートのURL', 'sfwj' ) }
					value={ attributes.spreadsheet }
					onChange={ ( spreadsheet ) =>
						setAttributes( { spreadsheet } )
					}
				/>
			);
		};
		return (
			<>
				<InspectorControls>
					<PanelBody
						defaultOpen={ true }
						title={ __( 'SFブロック設定', 'taro-taxonomy-blocks' ) }
					>
						<SpreadSheetUrl />
						<TextControl
							type="datetime-local"
							...
							onChange={ ( published_at ) =>
								setAttributes( { published_at } )
							}
							help={ __(
								'ページだけを先に公開しておき、ブロックを特定の日以降に公開したい場合は日時を指定してください。',
								'sfwj'
							) }
						/>
					</PanelBody>
